...
```json
{
  "hooks": {
    "PostToolUse": [{
      "command": "/Users/alice/code/cc-dev-team/hooks/check-messages.js"  // Must be absolute!
    }]
  },
  "permissions": {
...
| `hooks/check-messages.js` | Hook script: delivers pending messages between tool calls |
| `tools/ensure-poller.js` | Idempotent launcher: ensures one background poller is running |
| `tools/wait-for-messages.js` | Background poller: wakes idle agents when messages arrive (internal, spawned by ensure-poller) |
